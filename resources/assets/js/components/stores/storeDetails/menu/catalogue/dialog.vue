...
export default {
		...
		save: async function() {
			...
			if(vm.index > -1) {
				...
				vm.$validator.validateAll().then(async function(result){
					if(result) {
						await axios.post('/api/GetStore/'+vm.$route.params.storeId+'/Menu/Catalogue/Edit', data).then(response => {
				...
				vm.$validator.validateAll().then(async function(result){
					if(result) {
						await axios.post('/api/GetStore/'+vm.$route.params.storeId+'/Menu/Catalogue/Add', data).then(response => {
	...
	computed: {
		...mapState({
			dialog: state => state.catalogueStore.catalogueDialog,
			item: state   => state.catalogueStore.editedItem,
			index: state  => state.catalogueStore.editedIndex
		}),
		formTitle: function() {
			return this.index > -1 ? 'Chỉnh sửa danh mục' : 'Thêm danh mục' 
		}
	},
	watch: {
		'item': function(val) {
