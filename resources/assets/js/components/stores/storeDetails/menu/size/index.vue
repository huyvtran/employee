<template>
	<div>
		<v-card-title primary-title>
			<v-btn color="green darken-3" class="white--text" @click.native="$store.commit('SHOW_SIZE_DIALOG')" small round>
				Thêm mới
			</v-btn>
	...
	methods: {
		getSize: function(id) {
			return new Promise(async (resolve, reject) => {
				...
				await this.$store.dispatch('getSize', id)
				vm.loading = false
			})
		},
		editItem: function(item) {
			this.$store.commit('EDIT_SIZE', item)
		}
	},
	computed: {
		...mapState({
			items: state => state.sizeStore.sizes
		})
	},
	created() {
		this.getSize(this.$route.params.storeId)
	}
}
</script>